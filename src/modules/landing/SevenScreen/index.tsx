...
export const SevenScreen = () => {
  return (
    <main className={'section p-5 lg:pl-[50px] lg:pt-[70px] flex gap-16 xl:gap-10 ' + styles.screen}>
      <div className="lg:block flex flex-col" style={{ flex: 1 }}>
        <h1 className="text-3xl lg:text-4xl xl:text-5xl uppercase w-max">
          <span className={styles.titleSpan}>остались вопросы? </span> <br />
          Не знаете нужен ли вам <br />
          чат-бот/автоворонка?
        </h1>
        <h3 className="font-bold text-base lg:text-lg uppercase pt-6 pb-10 tracking-wider w-max">
        ...
        <div className={'rounded-xl p-6 xl:p-14 w-max ' + styles.feedback}>
          ...
          <div className="flex gap-6">
            <a className={'rounded-full px-8 py-6 xl:px-12 flex items-center text-base xl:text-lg uppercase font-medium  lg:gap-3 tracking-wide ' + styles.massagerLink}>
              <Telephone /> Позвонить
            </a>
            <a className={'rounded-full px-8 py-6 xl:px-12 flex items-center text-base xl:text-lg uppercase font-medium gap-2 lg:gap-3 tracking-wide ' + styles.massagerLink}>
              <TelegramFeedback /> Telegram
            </a>
          </div>
          <input className={'block w-full rounded-full text-base xl:text-lg px-12 py-6 my-6 ' + styles.feedbackInput} placeholder="ОСТАВЬТЕ КОНТАКТ" />
          <button className={'uppercase px-12 py-6 text-base xl:text-lg font-medium rounded-full ' + styles.cta}>получить консультацию</button>
        </div>
      </div>
