...
export const SecondScreen = () => {
  return (
    <main className={'section p-5 laptop:pl-[40px] laptop:pt-[70px] ' + styles.screen}>
      <h1 className={styles.mainTitle + ' text-3xl md:text-4xl laptop:text-5xl uppercase'}>
      ...
      <main className="flex flex-wrap pt-16 gap-5 content-between justify-center">
        ...
        <br className={'hidden desktop:block ' + styles.separator} />
        <CardShadow className="hidden desktop:block" />
        <CardShadow className={`hidden desktop:block ${styles.accentCard}`} />
        <Card
          title={'Рассылки CRM'}
          index={'05'}
          icons={
        ...
        <CardShadow className="block desktop:hidden">
          <CallToActionButton className="text-[18px] pt-0" textClassName="left-[32px] md:left-[32px] top-[25px]">
            Обсудить проект
          </CallToActionButton>
        </CardShadow>
...
const Card = ({
  ...
  return (
    <div className={`${styles.card} flex flex-col w-[320px] desktop:w-[400px] desktop:h-[540px] p-9 rounded-[35px]`}>
      <div className="flex flex-1 gap-2 align-middle" style={{ width: '40%' }}>
        {icons}
      </div>
      <h3 className={`${styles.number}  font-thin opacity-50`}>{index}</h3>
