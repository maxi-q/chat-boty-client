...
export const ThirdScreen = () => {
  return (
    ...
    >
      <h1 className={'relative phone:text-4xl tablet:text-6xl laptop:text-5xl desktop:text-6xl pb-16 uppercase tracking-wide ' + styles.title}>
      ...
      <h1 className={`relative phone:text-4xl tablet:text-6xl laptop:text-5xl desktop:text-6xl pb-8 pt-32 uppercase tracking-wide ${styles.subtitle}`}>и получите:</h1>
      <ul className={`flex relative gap-3 max-w-full mx-auto flex-col laptop:flex-row items-center laptop:justify-around h-max items-stretch ${styles.getBox}`}>
      ...
      <h1 className={`relative phone:text-4xl tablet:text-6xl laptop:text-6xl pb-8 pt-32 tracking-wide uppercase ${styles.subtitle}`}>итог:</h1>
      <div className="flex relative">
        <aside className={styles.markerResult} />
        <div className={`phone:text-lg tablet:text-2xl laptop:text-2xl laptop:w-[630px] ${styles.result}`}>
...
const Tablet = ({ icon, text, className, classCircle }: { icon: React.ReactNode; text: string; className: string; classCircle: string }) => {
  return (
    <div className={`${className} min-h-[138px] rounded-xl tablet:rounded-full phone:py-4 phone:px-3 laptop:p-4 gap-3 desktop:gap-10 bg-gray-900 w-full flex flex-col tablet:flex-row items-start tablet:items-center`} style={{ marginTop: '-3%' }}>
      <div className={`${classCircle} rounded-full phone:p-[10px] tablet:p-[38px] `}>{icon}</div>
      <p className="sm:leading-5 phone:text-base tablet:text-lg laptop:text-lg desktop:text-[18px] font-medium laptop:leading-[30px] tracking-wide">{text}</p>
    </div>
  )
}
...
const CardGet = ({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) => {
  return (
    <li
      className={`${styles.getCard} flex flex-col tablet:flex-row phone:py-4 tablet:py-3 laptop:py-6 phone:px-5 tablet:px-4 laptop:px-6 laptop:w-max w-full laptop:justify-center gap-3`}
      style={{ flex: 1 }}
    >
      ...
      <div className="flex flex-col justify-center">
        <h3 className="text-white phone:text-xl tablet:text-2xl laptop:text-2xl desktop:text-3xl uppercase nowrap">{title}</h3>
        <p className="text-white phone:text-sm tablet:text-lg laptop:text-lg desktop:text-xl font-hairline laptop:leading-relaxed tracking-wider mt-1 laptop:mt-[20px] tablet:whitespace-pre-wrap laptop:whitespace-normal">
          {text}
        </p>
      </div>
    </li>
