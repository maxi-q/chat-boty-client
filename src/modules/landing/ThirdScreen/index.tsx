...
export const ThirdScreen = () => {
  return (
    ...
    >
      <h1 className={'text-3xl relative tablet:text-6xl laptop:text-5xl desktop:text-6xl pb-16 uppercase tracking-wide ' + styles.title}>
        С Hами <br /> вы забудете о:
      </h1>
      <ul className="flex relative flex-col laptop:flex-row items-center laptop:items-start content-around w-full gap-[46px] laptop:gap-6" style={{ justifyContent: 'space-around' }}>
      ...
      <h1 className={`text-6xl relative laptop:text-5xl desktop:text-6xl pb-8 pt-32 uppercase tracking-wide ${styles.subtitle}`}>и получите:</h1>
      <ul className={`flex relative gap-3 max-w-full mx-auto flex-col laptop:flex-row items-center laptop:justify-around h-max items-stretch ${styles.getBox}`}>
        ...
        <CardGet icon={<GreenSettings />} title={'Большой функционал'} text={'Напишем свой скрипт, скопируем ЛЮБУЮ воронку,\nкоторая вам\nнравится и сделаем любые интеграции'} />
      </ul>

      <h1 className={`text-6xl relative laptop:text-6xl pb-8 pt-32 tracking-wide uppercase ${styles.subtitle}`}>итог:</h1>
      <div className="flex relative">
        <aside className={styles.markerResult} />
        <div className={`text-2xl laptop:text-2xl laptop:w-[630px] ${styles.result}`}>
  ...
  return (
    <li className={`${styles.getCard} py-3 laptop:py-6 px-4 laptop:px-6 laptop:w-max w-full flex laptop:justify-center gap-3`} style={{ flex: 1 }}>
      ...
      <div className="flex flex-col justify-center">
        ...
        <p
          className="text-white tablet:text-lg laptop:text-lg desktop:text-xl font-hairline laptop:leading-relaxed tracking-wider mt-1 laptop:mt-[20px] whitespace-pre-wrap laptop:whitespace-normal"
        >
          {text}
        </p>
      </div>
